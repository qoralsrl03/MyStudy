...
public class ExceptionMain {
	...
	public static void main(String[] args) {
		// 보통 예외상황에 대해서 에러가 발생했다고 표현함
		// 전문용어로 뻑났다고 함..
		// 예외는 굳이 나누면 예측 가능한 예외(개발자의 잘못이 없음)와,
		// 예측하지 못한 예외로 나눌 수 있다.
		...
		try {
			...
			// 해당 에러가 발생하면 catch 내부가 실행됨
			System.out.println("예외 발생 1");
			System.out.println("에러메시지 : " + e.getMessage());

			// 에러 로그 출력(항상 보던거)
			// System.err.println()으로 출력됨
			// System.out.println()랑 출력되는 타이밍이 지맘대로임
			e.printStackTrace();
		}
		System.out.println("인덱싱 종료 1");
...
//				break;
//			} else {
//				System.out.println("1, 2, 3중에 입력하지 않음");
...
//		}
		...
		// SELECT문 1줄 이상 출력
		// WHERE절을 걸었더니 SELECT 문 0줄
		// 시스템상 에러는 안남 -> 우리가 직접 에러 발생시킨다.

		// 충분히 예측되는 예외의 경우
		// 따로 Exception 클래스를 만들어서 다룬다.
		// 비즈니스 Exception -> BizException 과 같이 명명함
		try {
		...
		System.out.println("\n=====================================\n");
		ExMethod.dataMillSec("2023.04.20");

		System.out.println("\n=======================\n");
		try {
			ExMethod.printName("민기");
		} catch (BizException e) {
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
			...
			// 주로 try 블록 내에서 사용된 객체를 close 할 때 사용
